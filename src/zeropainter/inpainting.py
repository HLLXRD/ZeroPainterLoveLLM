...
def gen_filled_image(model, prompt, image, mask, zp_masks, seed, T = 899, dt = 20, model_t2i = None, guidance_scale = 7.5, use_lcm_multistep = False):
    ...
    decoded = model.vae.decode(z0 / model.config.scale_factor)  # Shape: (1, 3, H, W)

    # Squeeze batch dimension and scale to [0, 255]
    tensor_output = decoded.squeeze(0)  # Shape: (3, H, W)
    tensor_output = (tensor_output + 1) * 127.5  # Scale from [-1,1] to [0,255]
    tensor_output = tensor_output.clamp(0, 255).to(torch.uint8)

    return tensor_output  # Shape: (3, H, W), dtype=uint8
