...
def main(input_dir, maskcreation, zeropainter, faissmanager):
    answers = []
    scenes = os.listdir(input_dir)[1]  # get the second scene (just one folder name)
    scene_path = os.path.join(input_dir, scenes)  # full path to that scene folder
    ...
    for room_folder in tqdm(room_folders, desc="Processing rooms"):
        ...
        if text_path and image_path:
            ...
            try:
                object_dirs = process(room_folder, input_dir, input_text, image_path, maskcreation, zeropainter,
                                      faissmanager)
